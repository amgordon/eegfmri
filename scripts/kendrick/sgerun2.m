...
if isempty(jobindices)
  ...
  if isequal(jobindices,jobindices(1):jobindices(end))
...
qsubcmd = sprintf('qsub %s -N %s -o %s -e %s -p %d -l virtual_free=%dM %s -S /bin/sh -q %s %s %s; ',tstr,name,masterdir,masterdir,priority,memusage,flags,queuename,masterscript,name);
