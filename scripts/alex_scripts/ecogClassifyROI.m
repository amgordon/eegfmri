...
function ecogClassifyROI(expt, subjects, dur, shufl)

% this function sets up the necessary variables to perform binary
% classification on ecog data
%
...
for s = subjects
	...
	if strcmp(expt,'PC1')
		...
		if strcmp(s{1},'19') || strcmp(s{1},'20') ,S.expt = 'SS3';
		else S.expt = 'SS2';
		end
	end
	...
	S.rois = subject_rois_v2(S.subjnum); % channels by roi
	...
	S.trial_rejection_threh = 2; % trial rejection threshold
	S.trial_rejection_channels = 'LPC'; % use these channels to detect bad trials
	S.trial_rejection_band = 1; % band to based rejection, 1 is just amp data
	S.roisnames = S.rois.rois_present ; % rois that the subject has
	...
	temp = ecog_sub_contrasts(S.subjnum, S.expt);
	% contrasts
	S.contrasts = temp.interactions;
	S.contrastsnames = temp.interactions_names;
	
	
	% options for each toolbox
	if strcmp(S.toolbox, 'liblinear')
		addpath(genpath('liblinear-1.8/matlab/'))
	...
	elseif strcmp(S.toolbox, 'glmnet')
		...
		S.opts.alpha = 0.5; % L2-L1 parameter
		S.opts.nlambda = 100; % number of lambda
		S.opts.maxit = 100; % max number of iterations
		S.opts.perfmetric = S.perfmetric;
		S.solver_type = ['alpha' strrep(num2str(S.opts.alpha),'.','p')];
		S.lambdafolds = 10;  % lambda parameter selection folds.
	end
	
	% load the data and set up directories
	S.mainpath = '/biac4/wagner/biac3/wagner7/ecog'; % returns experimental directory
	datapath = [ S.mainpath '/subj' S.subjnum '/ecog/' S.expt];
	temp = load([datapath '/data_for_classification/allBandStruct_' 'binsize' num2str(S.binsize) '.mat']);
	S.data = temp.S; temp =[];
	
	
	savepath1 = [ '/Users/alex/Documents/research/MemoryLabResearch/ecog/' ...
		'Classification/' 'subj' S.subjnum '/classify_roi/' S.expt '/'];
	savepath2 = [ S.mainpath ...
		'/classification/data/subj' S.subjnum '/classify_roi/' S.expt '/'];
	mkdir(savepath1)
	mkdir(savepath2)
	...
	load([datapath '/BehavData/behav_perf.mat']);
	S.behavioral = behav_perf;
	...
	band = S.trial_rejection_band;
	X = squeeze(S.data.bin_trials(band,channels,:,:));
	...
	for cont = 1:size(S.contrasts,1);
		S.conds = S.contrastsnames(cont,:); % classification conditions
		S.cond_trials = [S.behavioral.(S.conds{1})&S.good_trials , ...
			S.behavioral.(S.conds{2})&S.good_trials];
		...
		for ro = 1:numel(S.roisnames)
			param_model=[];
			S.whichroi = S.roisnames{ro};
			S.channels = S.rois.(S.whichroi); % classify across channels
			S.nchan = numel(S.channels);
			
			filename1 = [savepath1 'subj' S.subjnum S.expt...
				S.contrastsnames{cont,1} '_' S.contrastsnames{cont,2} '_' S.toolbox 'solver' S.solver_type ...
				'_time' tc1 '_' tc2 '_' S.whichroi '_binsize' num2str(S.binsize)];
			filename2 = [savepath2 'subj' S.subjnum S.expt...
				S.contrastsnames{cont,1} '_' S.contrastsnames{cont,2} '_' S.toolbox 'solver' S.solver_type ...
				'_time' tc1 '_' tc2 '_' S.whichroi '_binsize' num2str(S.binsize)];
			...
			for band = [1:7]
				...
				display(sprintf('Classifying band#%i',band))
				
				% for each band calculate the mean sd of each channel
				X = S.data.bin_trials(band,S.channels,:,:);
				[nbands nch ntr nbins]=size(X);
				mX = mean(X(:,:,:),3);
				sdX = std(X(:,:,:),[],3);
				...
				S.N = size(S.X,1);
				S.nfolds = ceil(S.N/2); % number of cross validation folds
				...
				if save_now
					...
					save([filename1 shuflstr '.mat'],'param_model')
					save([filename2 shuflstr '.mat'],'param_model')
				end
