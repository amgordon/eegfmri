function [subj] = detrend_runs(subj,patname,selname,maskname,varargin)
...
defaults.new_patname = sprintf('%s_n0',patname);
defaults.new_maskname = sprintf('n0_%s',maskname);
args = propval(varargin,defaults);

pat = get_mat(subj,'pattern',patname);
sel = get_mat(subj,'selector',selname);

msk = get_mat(subj,'mask',maskname);
...
%msk(find(constant_rows),:)=0;


msk_zeros_idx = zeros(1,size(pat,1));

msk_zeros_idx(find(constant_rows))=1;



msk_incl_idx_found = find(msk==1);

msk_vox_to_delete = msk_incl_idx_found(find(msk_zeros_idx));

msk(msk_vox_to_delete) = 0;

pat(find(constant_rows),:)=[];

subj = duplicate_object(subj,'pattern',patname,args.new_patname);
subj = set_mat(subj,'pattern',args.new_patname,pat);

subj = set_objfield(subj,'pattern',args.new_patname,'masked_by', args.new_maskname);

zhist = sprintf('Pattern ''%s'' created by AG_remove_zero_voxels',args.new_patname);
subj  = add_history(subj,'pattern',args.new_patname,zhist,true);

created.function = 'AG_remove_zero_voxels';
...
subj = duplicate_object(subj,'mask',maskname,args.new_maskname);
subj = set_mat(subj,'mask',args.new_maskname,msk);


zhist = sprintf('Pattern ''%s'' created by AG_remove_zero_voxels',args.new_maskname);
subj  = add_history(subj,'mask',args.new_maskname,zhist,true);

created.function = 'AG_remove_zero_voxels';
created.patname = maskname;
created.selname = selname;
