...
if ~isempty(args.actives_group_name)
    activeSelNames = find_group(subj, 'selector', args.actives_group_name);
end
...
for r=unique(runs)
    ...
    if ~isempty(args.actives_group_name)
        actives = get_mat(subj,'selector', activeSelNames{r});
    end
