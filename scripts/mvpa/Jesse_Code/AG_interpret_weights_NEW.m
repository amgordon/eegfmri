function [subj] = AG_interpret_weights_NEW(subj,results,results_IW,S, varargin)
...
for i=1:nTimes
    ...
    for cds = 1:length(S.condnames)
        ...
        Vox.pos.(S.condnames{cds}).act = find(curCanonical{cds}>0);
        Vox.pos.(S.condnames{cds}).wts = find(curWts{cds}>0);
        Vox.pos.(S.condnames{cds}).sameSign = intersect(Vox.pos.(S.condnames{cds}).wts , Vox.pos.(S.condnames{cds}).act);
        
        Vox.neg.(S.condnames{cds}).act = find(curCanonical{cds}<0);
        Vox.neg.(S.condnames{cds}).wts = find(curWts{cds}<0);
        Vox.neg.(S.condnames{cds}).sameSign = intersect(Vox.neg.(S.condnames{cds}).wts , Vox.neg.(S.condnames{cds}).act);
        
        %Vox.both.(S.condnames{cds}).act = find(curCanonical{cds});
        %Vox.both.(S.condnames{cds}).wts = find(curWts{cds});
        %Vox.both.(S.condnames{cds}).sameSign = intersect(Vox.both.(S.condnames{cds}).wts , Vox.both.(S.condnames{cds}).act);
        
        Vox.both.(S.condnames{cds}).act = 1: length(curCanonical{cds});
        Vox.both.(S.condnames{cds}).wts = 1: length(curWts{cds});
        Vox.both.(S.condnames{cds}).sameSign = intersect(Vox.both.(S.condnames{cds}).wts , Vox.both.(S.condnames{cds}).act);
        
        curImp.pos.(S.condnames{cds}) = zeros(size(curWts{cds}));
        curImp.neg.(S.condnames{cds}) = zeros(size(curWts{cds}));
        curImp.both.(S.condnames{cds}) = zeros(size(curWts{cds}));
        
        curImp.pos.(S.condnames{cds})(Vox.pos.(S.condnames{cds}).sameSign)  = curWts{cds}(Vox.pos.(S.condnames{cds}).sameSign) .* curCanonical{cds}(Vox.pos.(S.condnames{cds}).sameSign);
        curImp.neg.(S.condnames{cds})(Vox.neg.(S.condnames{cds}).sameSign) = curWts{cds}(Vox.neg.(S.condnames{cds}).sameSign) .* curCanonical{cds}(Vox.neg.(S.condnames{cds}).sameSign);
        curImp.both.(S.condnames{cds})(Vox.both.(S.condnames{cds}).sameSign) = curImp.pos.(S.condnames{cds}) + curImp.neg.(S.condnames{cds});
        
        curImp.raw.(S.condnames{cds}) = curWts{cds}.*curCanonical{cds};
        
        %                 for it = 1:length(S.impType)
        %                 % identify voxels with positive activity and positive weights
        %
        %                     if strcmp('pos', S.impType{s})
        %                     Vox.(S.impType{it}).(S.condnames{cds}).act = find(curCanonical{cds}>0);
        %                     Vox.(S.impType{it}).(S.condnames{cds}).wts = find(curWts{cds}>0);
        %                     Vox.(S.impType{it}).(S.condnames{cds}).sameSign = intersect( Vox.(S.impType{it}).(S.condnames{cds}).act, Vox.(S.impType{it}).(S.condnames{cds}).wts);
        %                     elseif strcmp('neg', S.impType{s})
        %                     Vox.(S.impType{it}).(S.condnames{cds}).act = find(curCanonical{cds}>0);
        %                     Vox.(S.impType{it}).(S.condnames{cds}).wts = find(curWts{cds}>0);
        %                     Vox.(S.impType{it}).(S.condnames{cds}).sameSign = intersect( Vox.(S.impType{it}).(S.condnames{cds}).act, Vox.(S.impType{it}).(S.condnames{cds}).wts);
        %                     elseif strcmp('both', S.impType{s})
        %                     Vox.(S.impType{it}).(S.condnames{cds}).act = find(curCanonical{cds});
        %                     Vox.(S.impType{it}).(S.condnames{cds}).wts = find(curWts{cds});
        %                     Vox.(S.impType{it}).(S.condnames{cds}).sameSign = intersect( Vox.(S.impType{it}).(S.condnames{cds}).act, Vox.(S.impType{it}).(S.condnames{cds}).wts);
        %                     end
        %
        %                     FinalImp.(S.impType{it}).(S.condnames{cds}) = zeros(size(Vox.(S.impType{it}).(S.condnames{cds}).wts ));
        %
        %                 end
        ...
        importance.pos.(S.condnames{cds}).map = importance.pos.(S.condnames{cds}).map + curImp.pos.(S.condnames{cds});
        importance.neg.(S.condnames{cds}).map =  importance.neg.(S.condnames{cds}).map + curImp.neg.(S.condnames{cds});
        importance.both.(S.condnames{cds}).map = importance.both.(S.condnames{cds}).map + curImp.both.(S.condnames{cds});
        
        importance.raw.(S.condnames{cds}).map = importance.raw.(S.condnames{cds}).map + curImp.raw.(S.condnames{cds});
        %importance.raw(S.condnames{cds}.map = importance.raa(S.condnames{cds}).map
...
for it = 1:length(S.impType)
    for cds = 1:length(S.condnames)
        %for j=1:nRuns
        importance.(S.impType{it}).(S.condnames{cds}).map= importance.(S.impType{it}).(S.condnames{cds}).map;
        
        patname = strcat(args.impmap_name, '_', S.impType{it}, '_', S.condnames{cds} );
        subj = init_object(subj,'pattern',patname);
        subj = set_mat(subj,'pattern',patname,importance.(S.impType{it}).(S.condnames{cds}).map);
