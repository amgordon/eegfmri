...
yTrainSPM = load(fullfile(S.univar_dir, 'SPM'));
yTestSPM = load(fullfile(S.univar_dir, 'SPM'));

yTrain = load(fullfile(S.univar_dir, 'mvpa_ons'));
yTest = load(fullfile(S.univar_dir, 'mvpa_ons'));
...
if S.use_unsmoothed
    for i = 1:length(fnTrain)
        ...
        fnTrain{i} = ['/Users/alangordon/Studies/AG1/Accumulator_fMRI/AG1/fmri_data2' fnTrain{i}(50:end)];%to correct for change in base experimental path
    end
    
    for i = 1:length(fnTest)
        fnTest{i}(80) = [];
        
        fnTest{i} = ['/Users/alangordon/Studies/AG1/Accumulator_fMRI/AG1/fmri_data2' fnTest{i}(50:end)]; %to correct for change in base experimental path
    end    
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
onsets = {vertcat(onsets_h.onsets{S.onsetsIdx})};%make this dynamic
