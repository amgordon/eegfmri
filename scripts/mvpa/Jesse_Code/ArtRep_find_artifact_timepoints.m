function ArtRep_find_artifact_timepoints(subpar)
...
for k = 1:par.numscans
    ...
    M = []; P = [];
    for i = 1:num_sess
        P{i} = par.ascanfiles{k} %d.imagefilenames{subj}{i} %spm_select(Inf,'image',['Select data images for session'  num2str(i) ':']);
        if realignfile == 1
            
            
%             %%%%%%%
%             %%%Quick fix for subj s104
%             %%%%%%%
%             if k==9
%                 k=10
%             end
%             %%%%%%%%%%%
            ...
            mvmt_file =[par.funcdir '/' prepend(num2str(x), 2) '_func/rp_a' prepend(num2str(x), 2) '_func.V006.txt'];  
            M{i} = load(mvmt_file);  
...
save (filename, 'zscoreA_cell', 'delta_cell');
