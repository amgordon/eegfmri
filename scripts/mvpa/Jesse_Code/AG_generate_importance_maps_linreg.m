function AG_generate_importance_maps_linreg(subj, results, results_IW, S)

subj = AG_interpret_weights_linreg(subj, results,results_IW, S, 'impmap_name', 'betas');
...
for it = 1:length(S.impType)
    for cds = 1:length(S.condnames)
        ...
        thisPat = strmatch( 'betasRT' , patNames);
