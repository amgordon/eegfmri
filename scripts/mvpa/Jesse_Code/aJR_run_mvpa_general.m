function []= aJR_run_mvpa_general(subj_array, condition1, condition2, testcondition)

% example usage: aJR_run_mvpa_general([16],'ABonsetHighAB','ABonsetLowAB','ABonsettoTest')
for bigloop = 1:20
    ...
    for b=subj_array  % allows subject ID #'s to be specified in short format
        if b<10
            subj_id=strcat('RRIStudysub0', num2str(b));
        else
            subj_id=strcat('RRIStudysub', num2str(b));
        end

        %s
        expt_dir = '/Users/bkuhl/research_stuff/experiments/fMRI/RRI_scan';
    %     expt_dir = 'C:/mvpa'
        mvpa_dir = [expt_dir '/MVPAarpeet/' subj_id];
        importance_maps_dir=[mvpa_dir '/mvpa_results/improved_importance_maps/' condition1 '_vs_' condition2];
        xls_results_data_logs_dir=[mvpa_dir '/mvpa_results/xls_results_data_logs/' condition1 '_vs_' condition2];

    %     roi_name = 'merged_AAL_ROIs_FIXED_HOLES';  % name of anatomical mask -- use spm mask ones, fix holes
        roi_name = 'thresh_ABreward'; % changed ROI mask name
    %     data_imgs_to_use = 'raw_filenames_swa.mat'; % .mat file containing names of all functional images
        data_imgs_to_use = aJR_make_mvpa_rawfilenames_list(subj_id);
        ...
        load([expt_dir '/MVPAarpeet/vol_info.mat']); %get functional data resolution info for spm .img writing
       % load(['/Users/bkuhl/research_stuff/experiments/fMRI/RRI_scan/fmri_data/study/' subj_id '/behavioral_model9/onsetsRemovedACFixed.mat']); % load in your SPM-formatted onsets file
        load(['/Users/bkuhl/research_stuff/experiments/fMRI/RRI_scan/fmri_data/study/' subj_id '/behavioral_model9/onsetstostims.mat']); % load in your SPM-formatted onsets file
        %     load('onsetsRemovedACFixed.mat');

        % OPTIONAL:  specify previously saved mvpa workspace to bypass time-consuming data extraction and preprocessing
        mvpa_workspace = [expt_dir '/MVPAarpeet/' subj_id '/mvpa_results/subjworkspace.mat'];
    %     
    ...
    %     flags.equate_number_of_old_new_trials_per_subjective_bin = 1; % equate number of trials per subjective bin
        ...
        flags.remove_artdetect_outliers = 0; % BK: changed to 0 1 = remove trials that exhibited movement or global signal artifacts as determined by ArtDetect
        ...
        flags.remove_outlier_trials = 0; % BK: was 4, brice made 0 on-the-fly outlier detection/removal; specify how many std dev from whole brain mean to exclude as outliers (0 = don't exclude any trials)
        ...
        flags.junk_cond = 7; % AS: added to filter out junk condition
        ...
        if bigloop<5
            TR_weights = [1 0 0 0 0]; % should sum to 1
        elseif bigloop<9
            TR_weights = [0 1 0 0 0];
        elseif bigloop<13
            TR_weights = [0 0 1 0 0];
        elseif bigloop<17
            TR_weights = [0 0 0 1 0];
        elseif bigloop<21
            TR_weights = [0 0 0 0 1];
        else
            display('???')
            keyboard
        end
        ...
        if ~exist(mvpa_workspace,'file')
            [subj num_runs num_TP_per_run]= aJR_generate_mvpa_workspace_mat_file(subj_id, roi_name, data_imgs_to_use, mvpa_dir); % generate and save workspace
        else
        ...
        for n = 1: flags.num_results_iter
            ...
            all_regs = zeros(num_conds,sum(num_TP_per_run)); % initialize regs matrix as conditions x timepoints

            for cond = 1: num_conds %(exclude last condition ("no_response" trials) % AS changed to loop through all conditions
                for trial = 1: length(onsets{cond})
                    if cond~=flags.junk_cond            % AS added, doesn't include junk condition
    ...
    %         end
            ...
            % AS: changed the conditions to be based off the following:
            % specify names        1'H.H.o.o.o.AB.S'    2'H.L.o.o.o.AB.S'
            % 3'H.x.o.o.o.AB.S'    4'L.H.o.o.o.AB.S'    5'L.L.o.o.o.AB.S'
            % 6'L.x.o.o.o.AB.S'    7'J.J.o.o.o.AB.S'    8'H.H.o.o.o.AC.S' 
            % 9'H.L.o.o.o.AC.S'   10'L.H.o.o.o.AC.S'  11'L.L.o.o.o.AC.S'

            highshuff = shuffle([8 10]);
            lowshuff = shuffle([9 11]);
            ABonsettoTest =  sum(condensed_regs_all([highshuff(1) lowshuff(1)],:));

            ABonsetHighAB = sum(condensed_regs_all([1 2 3 highshuff(1)],:));
            ABonsetLowAB = sum(condensed_regs_all([4 5 6 lowshuff(1)],:));

    %         AConsetHighAB = sum(condensed_regs_all([8 9],:));
    %         AConsetLowAB = sum(condensed_regs_all([10 11],:));
    %         
    %         AConsets = sum(condensed_regs_all([8 9 10 11],:));
    %         
    %         AConsetHighAC = sum(condensed_regs_all([8 10],:));
    %         AConsetLowAC = sum(condensed_regs_all([9 11],:));
    %         
    %         AllHighABMinusHighAC = sum(condensed_regs_all([1 2 3 9],:));
    %         AllLowABMinusHighAC = sum(condensed_regs_all([4 5 6 11],:));
    %         
    %         AllHighABMinusLowAC = sum(condensed_regs_all([1 2 3 8],:));
    %         AllLowABMinusLowAC = sum(condensed_regs_all([4 5 6 10],:));
    %                 
    %         AllonsetsHighAB = sum(condensed_regs_all([1 2 3 8 9],:));
    %         AllonsetsLowAB = sum(condensed_regs_all([4 5 6 10 11],:));
    % 
    %         AllonsetsHigh = sum(condensed_regs_all([1 2 3 8 10],:));
    %         AllonsetsLow = sum(condensed_regs_all([4 5 6 9 11],:));
            ...
            % AS commented out: no need for active trials: all are active
    %         active_trials = find(sum(condensed_regs_of_interest));

            if flags.remove_outlier_trials ~= 0
                % remove outlier trials (timepoints)
                mean_across_voxels = mean(subj.patterns{end}.mat(:,active_trials),1);  % BK: what is this active_trials variable?
            ...
            %%%%%% this section makes the selector for the RRI experiment 
            active_trials = find(sum(condensed_regs_of_interest)); % this will be all trials 
            tests_selector = zeros(1,size(condensed_regs_all,2)); % intialize vector of all zeros
            tests_selector(active_trials) = 1; % set all trials to 1 initially 
            tests_selector(find(eval(testcondition))) = 2; % and set test trials to 2


            subj = init_object(subj,'selector','testselector');
            subj = set_mat(subj,'selector','testselector',tests_selector);
            subj = set_objfield(subj,'selector','testselector','group_name','testgroup','ignore_absence',true);

                % Tell it the story of how it came to be
                created.function = 'mvpa_general';
                subj = add_created(subj,'selector','testselector',created);

    %       AS commented out -- no need for actives -- all trials will be used
    %       and no need for the n-1 paradigm here
    ...
    %         subj = create_xvalid_indices(subj,'runs','actives_selname','conditions_of_interest');
            ...
            if flags.anova_p_thresh ~= 1
                subj = feature_select(subj,'spiral_d_hp_z_condensed','conds','testgroup','thresh',flags.anova_p_thresh);
            ...
            if exist('anova_nVox_thresh','var')
                subj = JR_feature_select_top_N_vox(subj,'spiral_d_hp_z_condensed','conds','testgroup','nVox_thresh',anova_nVox_thresh);
                classifier_mask = subj.masks{end}.group_name; % use group of masks created by ANOVA
            else
                classifier_mask = subj.masks{1}.name; % use original mask
            end
            ...
            for p = 1:flags.num_iter_with_same_data
                ...
                % AS commented out to exclude runs_xval and include
                % tests_selector instead
    %             [subj results{x}] = cross_validation(subj,'spiral_d_hp_z_condensed','conds','runs_xval',classifier_mask,class_args);
                [subj results{x}] = cross_validation(subj,'spiral_d_hp_z_condensed','conds','testgroup',classifier_mask,class_args);
                ...
                if flags.display_performance_breakdown == 1
                    ...
                    overall_accuracy(bigloop) = mean(correct_vector)*100;
                    acclist = overall_accuracy'
                    
                    % BK: added to save file
                    cmd = ['save acclist_' subj_id(9:end) '.mat acclist'];
                    eval(cmd);
                    
    %                 BK: commented out the below
    %                 overall_hit_rate =
    %                 mean(correct_vector(desireds_vector==1))
