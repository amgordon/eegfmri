function [qqq, results]= DKSort_run_mvpa_bySess(subj_array, saveName)
    ...
    for penal = 1:length(S.PenaltyParams)
        ...
        for CI = 1:length(S.condsOfInterestList)
            ...
            for b=(1:length(subj_array))  % allows subject ID #'s to be specified in short format
                    ...
                    S.wascanfiles = mat2cell(par.wascanfiles{ss}, ones(1, size(par.wascanfiles{ss},1)), size(par.wascanfiles{ss},2));
                    S.swascanfiles = mat2cell(par.swascanfiles{ss}, ones(1, size(par.swascanfiles{ss},1)), size(par.swascanfiles{ss},2));    
                    ...
                    S.runs_vector = [96 96 96 96 96 (par.numvols(ss) - 96*5)];
                    
%                     for i = 1:6
%                         S.runs_vector(i) = round(i*par.numvols(ss)/6) - round((i-1)*par.numvols(ss)/6);
%                     end
%                     
                    ...
                    S.condsOfInterest{1} = [S.condsOfInterestList{CI}{1} '_Session' num2str(ss)];
                    S.condsOfInterest{2} = [S.condsOfInterestList{CI}{2} '_Session' num2str(ss)];
                    %s
                    S.expt_dir = '/Users/alangordon/DK_sort/fmri_data';
                    S.mvpa_dir = [S.expt_dir '/' S.subj_id '/mvpa_analysis_bySess'];
                    ...
                    for n = 1: flags.num_results_iter
                        ...
                        for p = 1:flags.num_iter_with_same_data
                            ...
                            qqq.test{CI}.penalty(penal).FeatureSel(FS).subj{b}.Sess(ss).iter{n} = results{1};
