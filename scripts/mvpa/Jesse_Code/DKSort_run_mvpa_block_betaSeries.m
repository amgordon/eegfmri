function [qqq, results]= DKSort_run_mvpa_block(subj_array, saveName)
...
%S.condsOfInterestList = {{'CorTrial_shape_same_match', 'CorTrial_shape_diff_match'}; {'CorTrial_shape_same_nonmatch', 'CorTrial_shape_diff_nonmatch'};{'CorTrial_color_same_match', 'CorTrial_color_diff_match'}; {'CorTrial_color_same_nonmatch', 'CorTrial_color_diff_nonmatch'};{'CorTrial_pattern_same_match', 'CorTrial_pattern_diff_match'}; {'CorTrial_pattern_same_nonmatch', 'CorTrial_pattern_diff_nonmatch'}};
%S.condsOfInterestList = {{'Cue_shape', 'Cue_color'} {'Cue_shape', 'Cue_pattern'}  {'Cue_color', 'Cue_pattern'} {'Cue_same', 'Cue_different'}};
S.condsOfInterestList = {{'cue_shape_same' 'cue_shape_diff'}; {'cue_color_same' 'cue_color_diff' }; {'cue_pattern_same' 'cue_pattern_diff' }; {'cue_shape_same' 'cue_color_same'}; {'cue_shape_same' 'cue_pattern_same'}; {'cue_color_same' 'cue_pattern_same'}; {'cue_shape_diff' 'cue_color_diff'}; {'cue_shape_diff' 'cue_pattern_diff'}; {'cue_color_diff' 'cue_pattern_diff'}} ;
%S.condsOfInterestList = { {'Cue_same', 'Cue_different'}};
S.PenaltyParams = [100];
S.featureSels = [1000];
    ...
    for penal = 1:length(S.PenaltyParams)
        ...
        for CI = 1:length(S.condsOfInterestList)
            ...
            for b=(1:length(subj_array))  % allows subject ID #'s to be specified in short format
                ...
                %s
                S.expt_dir = '/Users/alangordon/DK_sort/fmri_data';
                S.mvpa_dir = [S.expt_dir '/' S.subj_id '/mvpa_analysis_12trials_6cues'];
                S.univar_dir = [S.expt_dir '/' S.subj_id '/analysis_12trials12_6cues'];
                ...
                S.univar_dir.BS = [S.expt_dir '/' S.subj_id '/analysis_CueBlocks_betaSeries'];
                ...
                S.equate_number_of_trials_in_cond_1_and_2 = 0; % equate number of trials in conditions 1 and 2 (RECOMMENDED)
                ...
                S.TR_weights_idx  = [0 0 ones(1,22)/22]; %block design; weight 3rd-24th TR equally
                ...
                if (flags.use_premade_workspace&&existWorkspace) %(flags.use_premade_workspace && Sequal) %
                    ...
                    if flags.betaSeriesAnalysis == 1;
                        [subj]= DK_mvpa_load_betaSeries(S);
                    else
...
%                     subj = remove_mat(subj,'pattern','spiral_d_hp_z');
                    ...
                    if S.class_args.nVox>0
                        subj = AG_feature_select_top_N_vox(subj,'betas','conds',S.thisSelector,'nVox_thresh',S.class_args.nVox,'statmap_arg',statmap_arg);
                        classifier_mask = subj.masks{end}.group_name; % use group of masks created by ANOVA
                    end
                    %%%%%%%%
                    ...
                    for p = 1:flags.num_iter_with_same_data
                        ...
                        [subj results{x}] = cross_validation(subj,'betas','conds', S.thisSelector ,classifier_mask,S.class_args); %modified specifically for Enc/Ret, 081709
