...
for sOI = 1:length(S.onsetsIdxSet)
    ...
    for b=1:length(subj_array)  % allows subject ID #'s to be specified in short format
        ...
        S.importance_maps_dir=[S.expt_dir '/mvpa_results/ImpMaps_050310/' [S.thisTrain S.thisTest] ];
        ...
        S.class_args.train_funct_name = 'train_ridge';
        S.class_args.test_funct_name = 'test_ridge';
...
%             display([num2str(count(condensed_regs_of_interest(2,:)==1)) ' trials in condition ' S.condnames{2}])
            ...
            subj = duplicate_object(subj,'pattern','spiral_n0_d_hp_z','spiral_n0_d_hp_z_condensed');
            subj = set_mat(subj,'pattern','spiral_n0_d_hp_z_condensed',temporally_condensed_data,'ignore_diff_size',true);
            
            zhist = sprintf('Pattern ''%s'' created by JR custom code','spiral_n0_d_hp_z_condensed');
            subj = add_history(subj,'pattern','spiral_n0_d_hp_z_condensed',zhist,true);
            
            % clean up workspace to save RAM
            subj = remove_mat(subj,'pattern','spiral_n0_d_hp_z');
            ...
            FSMasks = subj.masks;
            save ([S.mvpa_dir '/FS_LinRegRTs'], FSMasks);
