function [out]=AGlogRegFun(y,x,lambda,tol,maxrounds,const)
...
if const
    LL = @(w)(y*(w'*x)'-sum(log(1+exp(w'*x)))-lambda/2*w(2:end,:)'*w(2:end,:));
    C2 = lambda*eye(nFeat);
    C2(1,1) = 0;
else
...
while deltaLL>tol & rounds<maxrounds
  ...
  if const
      wGrad = B \ (x*(y-p)'-[0 * wOld(1,:); lambda*wOld(2:end,:)]); %changed by AG
  else
